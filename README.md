Для запуска frontend проекта в режиме разработки необходимо:
...
Первый подход использует NgRx store для хранения данных, к которым можно обратиться с помощью селекторов.
...
NgRx хорошо сочетается со стратегией OnPush при условии что мутируемые типы данных внутри основного объекта NgRx не будут мутироваться, а будет переписываться ссылка на них. 
В этом случае стратегия OnPush обнаружит изменение входящих в компонент данных (что ссылка на мутируемый тип данных изменилась) и компонент помечается как dirty и перерисуется при следующем тике. 
Либо при подписке на селектор можно запустить detectChanges(), инициировать tick и перерисовать компонент.

В модуле admin используется другой подход написания код. При инициализации компонента модель наполняется данными и привязывается к шаблону. 
...
В админском модуле демонстрируется работа с pure(чистым) фильтром, что позволяет снизить нагрузку при перерисовке шаблона и повысить производительность сайта.
...
В разметке активно используется библиотека Bootstrap 5. Если товар уже находится в корзине, то меняется иконка (реализовано с помощью селекторов и атрибутивной директивы ngClass). Демонстрируется пример использования @Viewchild и @Viewchildren.

Клиентская часть проекта адаптированна под мобильные устройства. Реализован screen-сервис, в котором удачно демонстрируется использование BehaviorSubject'а, который передает значение ширины окна страницы. Это используется для отображения мобильной версии верстки.

При фильтрации товаров (в категории товаров) используется библиотека RxJS (такие операторы как fromEvent(),distinctUntilChanged(),debounceTime(300)), что позволяет уменьшить количество запросов на бэкэнд. 
